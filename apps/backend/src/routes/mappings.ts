  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const splitterOutput =
        (r["SALIDA SPLITTER"] ?? r["SALIDA"] ?? r["SPLITTER_SALIDA"] ?? "")
          .toString()
          .trim() || undefined;
      ...
      if (slot === 9 || slot === 10) {
        ...
        errors.push({
          row: rowNumber,
          error:
            "Slot 9/10 son controladoras; no se crea Port. Use otro slot para PON.",
        });
        continue;
      }
      ...
      try {
        ...
        errors.push({
          row: rowNumber,
          error: "Error procesando fila (consulte logs)",
        });
      }
    }
