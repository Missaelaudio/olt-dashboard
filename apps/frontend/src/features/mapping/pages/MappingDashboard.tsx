      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-xl bg-slate-900 flex items-center justify-center shadow-lg shadow-brand-purple/5">
        ...
        <div className="flex gap-2">
          <Badge className="bg-slate-100 text-slate-600 border-none font-bold uppercase tracking-[0.15em] text-[10px] px-4 py-2 rounded-lg shadow-none">
            Formato: .XLSX
          </Badge>
          <Badge className="bg-emerald-50 text-emerald-600 border-none font-bold uppercase tracking-[0.15em] text-[10px] px-4 py-2 rounded-lg shadow-none">
            Estado: Seguro
          </Badge>
        </div>
      </div>
          ...
          <div className="enterprise-card p-8 group">
            ...
            <form onSubmit={handleUpload} className="space-y-8">
              <div
                ...
                ${
                  file
                    ...
                    : "border-slate-200 bg-slate-50/30 hover:bg-slate-50 hover:border-brand-purple/30"
                }
              `}
              >
                <input
                ...
                <div
                  ...
                  ${file ? "bg-emerald-500 text-white" : "bg-white text-brand-purple border border-slate-200 shadow-sm"}
                `}
                >
                  {file ? (
                ...
                <div className="space-y-2">
                  <p className="text-[14px] font-bold text-slate-900 uppercase tracking-wide">
              ...
              <Button
                ...
                className="w-full h-14 bg-slate-900 hover:bg-black text-white font-bold rounded-xl transition-all active:scale-[0.98] disabled:opacity-20 text-[13px] uppercase tracking-[0.2em]"
              >
                {loading ? (
              ...
              {[
                ...
                <div key={field.id} className="space-y-2.5">
                  <Label
                    ...
                    className="text-[11px] font-bold text-slate-500 uppercase tracking-widest ml-1"
                  >
                    {field.label}
                  </Label>
                  <Input
                    ...
                    className="h-12 border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white focus:ring-4 focus:ring-slate-900/5 transition-all text-[14px] font-semibold"
                  />
                </div>
              ))}
      ...
      {(message || (result && result.errors?.length > 0)) && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 z-50 animate-in fade-in slide-in-from-bottom-8 duration-500">
          ...
          {result && result.errors?.length > 0 && (
            <div className="w-[650px] max-h-[220px] overflow-auto grid grid-cols-2 gap-3 p-4 bg-white rounded-2xl border border-slate-200 shadow-2xl scrollbar-hide animate-in zoom-in-95">
              {result.errors.slice(0, 10).map((err: any, idx: number) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 text-[12px] bg-slate-50 rounded-xl px-4 py-3 border border-slate-100 group hover:border-slate-300 transition-all"
                >
