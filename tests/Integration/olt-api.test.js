...
function createTempExcel(rows, filename = 'ports.xlsx') {
...
describe('OLT API integration', () => {
    beforeAll(() => __awaiter(void 0, void 0, void 0, function* () {
        const res = yield request(app).post('/api/olts').send({ name: 'GRN-OLT-Test' });
        expect(res.status).toBe(200);
        oltId = res.body.id;
        expect(oltId).toBeTruthy();
    }));
    it('rechaza Excel con slot prohibido 9', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    it('rechaza Excel con puerto fuera de rango', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        expect(res.status).toBe(200);
        expect(res.body.inserted).toBe(0);
        expect(res.body.errors.some((e) => e.error.includes('fuera de rango'))).toBe(true);
    }));
    it('sube Excel válido e inserta puertos', () => __awaiter(void 0, void 0, void 0, function* () {
        const rows = [
    ...
    it('reemplaza puertos previos al subir nuevo Excel', () => __awaiter(void 0, void 0, void 0, function* () {
        // Primer Excel con 2 puertos
        const filePath1 = createTempExcel([
            { slot: 1, portNumber: 1, label: 'A' },
            { slot: 1, portNumber: 2, label: 'B' },
        ]);
        yield request(app).post(`/api/olts/${oltId}/ports`).attach('file', filePath1);
        // Segundo Excel con 1 puerto (debe reemplazar los anteriores)
        const filePath2 = createTempExcel([{ slot: 2, portNumber: 1, label: 'C' }]);
        const res = yield request(app).post(`/api/olts/${oltId}/ports`).attach('file', filePath2);
        expect(res.status).toBe(200);
        expect(res.body.inserted).toBe(1);
        // Verificar que solo existe el puerto nuevo
        const portsRes = yield request(app).get(`/api/olts/${oltId}/ports`);
        expect(portsRes.body.length).toBe(1);
        expect(portsRes.body[0].label).toBe('C');
    }));
    it('devuelve puertos del OLT sin incluir slots prohibidos', () => __awaiter(void 0, void 0, void 0, function* () {
        const res = yield request(app).get(`/api/olts/${oltId}/ports`);
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body)).toBe(true);
        const hasProhibited = res.body.some((p) => p.slot === 9 || p.slot === 10);
